(function () {
  ...
  const TOOLTIP_CSS_URL = chrome.runtime.getURL('content.css');
  let TOOLTIP_CSS = '';

  // Load external CSS asynchronously on init
  fetch(TOOLTIP_CSS_URL)
    .then((r) => r.text())
    .then((css) => { TOOLTIP_CSS = css; })
    .catch(() => {});
  let hostRoot = null;
  let tooltip = null;
  let controls = null;
  ...
  function buildRecord(el) {
    ...
    return {
      ...
      fontWeight: weightName(style.fontWeight),
      lineHeight,
      color,
    ...
    t.innerHTML = `
      ...
      <div class="typepeek-grid">
        ...
        <div class="typepeek-cell">
          ...
          <span class="typepeek-value">${weightName(style.fontWeight)}</span>
        </div>
        <div class="typepeek-cell">
  ...
  document.addEventListener('mousemove', (e) => {
    ...
    const now = Date.now();
    if (now - lastTooltipUpdate < 80) return;
    ...
    if (textEl === currentTarget) return;
  ...
  // Only create floating bar in the top-level frame.
